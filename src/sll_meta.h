...
#define SLL_DECLS(function_prefix, node_typename, list_typename) \
	...
	void           CONCAT(function_prefix, lclear)   (list_typename *list); \
	void           CONCAT(function_prefix, lnclear)  (node_typename *node); \
	size_t         CONCAT(function_prefix, lsize)    (const list_typename *list); \
	void           CONCAT(function_prefix, lpushback)(list_typename *list, node_typename *node); \
	node_typename *CONCAT(function_prefix, lpopfront)(list_typename *list); \
	void           CONCAT(function_prefix, lfree)(list_typename *list)
...
#define SLL_ITER_DECLS(function_prefix, node_typename, list_typename, iterator_typename) \
	...
	void           CONCAT(function_prefix, istart)(iterator_typename *iter, list_typename *list); \
	node_typename *CONCAT(function_prefix, iget)  (iterator_typename *iter); \
	void           CONCAT(function_prefix, inext) (iterator_typename *iter); \
	bool           CONCAT(function_prefix, iisend)(const iterator_typename *iter); \
	node_typename *CONCAT(function_prefix, ipop)  (iterator_typename *iter)

#define SLL_POOL_DECLS(function_prefix, node_typename, list_typename, pool_typename) \
	typedef list_typename pool_typename; \
	node_typename *CONCAT(function_prefix, pget)   (pool_typename *pool); \
	void           CONCAT(function_prefix, preturn)(pool_typename *pool, node_typename *node); \
	void           CONCAT(function_prefix, pfree)(pool_typename *pool)
...
#define SLL_DEFS(function_prefix, node_typename, list_typename, node_free_func) \
	...
	void CONCAT(function_prefix, lfree)(list_typename *list) { /*{{{*/ \
		while (CONCAT(function_prefix, lsize)(list) > 0) { \
			node_typename * node = CONCAT(function_prefix, lpopfront)(list); \
			node_free_func(node); \
		} \
	} /*}}}*/
...
#define SLL_POOL_DEFS(function_prefix, node_typename, list_typename, pool_typename) \
	node_typename *CONCAT(function_prefix, pget)(pool_typename *pool) { /*{{{*/ \
		assert(pool != NULL); \
		node_typename *ret = CONCAT(function_prefix, lpopfront)((list_typename*)pool); \
		if (ret == NULL) { \
	...
	void CONCAT(function_prefix, preturn)(pool_typename *pool, node_typename *node) { /*{{{*/ \
		assert(pool != NULL); \
		assert(node != NULL); \
		CONCAT(function_prefix, lpushback)((list_typename*)pool, node); \
	} /*}}}*/ \
	void CONCAT(function_prefix, pfree)(pool_typename *pool) { /*{{{*/ \
		assert(pool != NULL); \
		CONCAT(function_prefix, lfree)((list_typename*)pool); \
	} /*}}}*/
